...
// Usa sesion de servidor (express-session) en vez de JWT porque es mas simple
// de revocar (contratos que caducan, bloquear una cuenta, etc).
//
// Las cuentas nuevas quedan PENDIENTES de aprobacion: el admin las activa a
// mano desde su panel (Clientes -> "Aprobar cuenta"). No se envia ningun
// correo. La cuenta con rol admin no necesita aprobacion.
//
...
router.post('/register', registerLimiter, async (req, res, next) => {
  try {
    ...
    // No se abre sesion: la cuenta queda pendiente hasta que el admin la apruebe.
...
router.post('/login', loginLimiter, async (req, res, next) => {
  try {
    ...
    // El admin nunca necesita aprobacion; el resto, si.
    if (!user.emailVerified && user.role !== 'admin') {
      return fail(res, 403, 'ACCOUNT_PENDING', 'Tu cuenta esta pendiente de aprobacion.');
    }
